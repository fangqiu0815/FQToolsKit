...
{
    static FQUpdateManage * updateManager = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        updateManager.appStoreUrl = [[NSString alloc] initWithFormat:@"http://itunes.apple.com/lookup?id=%@",kAPPID];
        //
...
{
    FQUpdateManage *manager = [FQUpdateManage sharedInstance];
...
- (void)showAlert:(BOOL)isforce {
    ...
    UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:kUPDATETITLEMESSEGE message:[FQUpdateManage sharedInstance].updateMessage preferredStyle:UIAlertControllerStyleAlert];
    
    UIViewController *cv = [self getCurrentVC];
    
    UIAlertAction *ok = [UIAlertAction actionWithTitle:kUPDATECONFIRM style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        //跳转到appStore 须真机测试看效果 适配8系统用这个老的跳转
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://itunes.apple.com/app/id%@?mt=8", kAPPID]]];
    ...
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:kUPDATECANCEL style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        //异步操作吧 文件写入有点费时
    ...
    if (isforce) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //看是否弹出最大次数 弹出窗体
                if (chanelCount<kMAXUPDATECOUNT) {
                    //添加按钮
                    [alertVc addAction:ok];
