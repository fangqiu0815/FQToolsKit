...
#define kFQIndexViewSpace (self.configuration.indexItemHeight + self.configuration.indexItemsSpace)
#define kFQIndexViewMargin ((self.bounds.size.height - kFQIndexViewSpace * self.dataSource.count) / 2)

static NSTimeInterval kAnimationDuration = 0.25;
static void * kFQIndexViewContext = &kFQIndexViewContext;
static NSString *kFQFrameStringFromSelector = @"frame";
static NSString *kFQCenterStringFromSelector = @"center";
static NSString *kFQContentOffsetStringFromSelector = @"contentOffset";

// 根据section值获取CATextLayer的中心点y值
static inline CGFloat FQGetTextLayerCenterY(NSUInteger position, CGFloat margin, CGFloat space)
{
    return margin + (position + 1.0 / 2) * space;
}

// 根据y值获取CATextLayer的section值
static inline NSInteger FQPositionOfTextLayerInY(CGFloat y, CGFloat margin, CGFloat space)
{
    CGFloat position = (y - margin) / space - 1.0 / 2;
    if (position <= 0) return 0;
    NSUInteger bigger = (NSUInteger)ceil(position);
    NSUInteger smaller = bigger - 1;
    CGFloat biggerCenterY = FQGetTextLayerCenterY(bigger, margin, space);
    CGFloat smallerCenterY = FQGetTextLayerCenterY(smaller, margin, space);
    return biggerCenterY + smallerCenterY > 2 * y ? smaller : bigger;
}

@interface FQIndexesView ()
...
- (instancetype)initWithTableView:(UITableView *)tableView configuration:(FQIndexesViewConfig *)configuration
{
    if (self = [super initWithFrame:tableView.frame]) {
        ...
        [tableView addObserver:self forKeyPath:kFQFrameStringFromSelector options:NSKeyValueObservingOptionNew context:kFQIndexViewContext];
        [tableView addObserver:self forKeyPath:kFQCenterStringFromSelector options:NSKeyValueObservingOptionNew context:kFQIndexViewContext];
        [tableView addObserver:self forKeyPath:kFQContentOffsetStringFromSelector options:NSKeyValueObservingOptionNew context:kFQIndexViewContext];
    }
    return self;
}
...
- (void)dealloc
{
    [self.tableView removeObserver:self forKeyPath:kFQFrameStringFromSelector];
    [self.tableView removeObserver:self forKeyPath:kFQCenterStringFromSelector];
    [self.tableView removeObserver:self forKeyPath:kFQContentOffsetStringFromSelector];
}
...
- (void)configSubLayersAndSubviews
{
    ...
    CGFloat space = kFQIndexViewSpace;
    CGFloat margin = kFQIndexViewMargin;
    
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    
    if (hasSearchLayer) {
        self.searchLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight, FQGetTextLayerCenterY(0, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight, self.configuration.indexItemHeight);
    ...
    for (int i = 0; i < self.subTextLayers.count; i++) {
        ...
        textLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight, FQGetTextLayerCenterY(section, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight, self.configuration.indexItemHeight);
    ...
    if (self.subTextLayers.count == 0) {
        ...
        self.currentSection = self.searchLayer ? FQIndexViewSearchSection : 0;
    } else {
...
{
    NSInteger currentSection = FQIndexViewInvalidSection;
    if (self.delegate && [self.delegate respondsToSelector:@selector(sectionOfIndexView:tableViewDidScroll:)]) {
        ...
        if ((currentSection >= 0 && currentSection != FQIndexViewInvalidSection)
            || currentSection == FQIndexViewSearchSection) {
    ...
    if (currentSection < 0 && currentSection != FQIndexViewSearchSection) return;
    self.currentSection = currentSection;
}
...
{
    if (context != kFQIndexViewContext) return;
    
    if ([keyPath isEqualToString:kFQCenterStringFromSelector] || [keyPath isEqualToString:kFQFrameStringFromSelector]) {
        ...
        [CATransaction commit];
    } else if ([keyPath isEqualToString:kFQContentOffsetStringFromSelector]) {
...
{
    if ((self.currentSection < 0 && self.currentSection != FQIndexViewSearchSection)
        || self.currentSection >= (NSInteger)self.subTextLayers.count) {
        return;
    }
    
    if (self.currentSection == FQIndexViewSearchSection) {
...
- (void)showIndicator:(BOOL)animated
{
    ...
    if (self.configuration.indexViewStyle == FQIndexViewStyleDefault) {
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    NSInteger currentPosition = FQPositionOfTextLayerInY(location.y, kFQIndexViewMargin, kFQIndexViewSpace);
...
{
    if ((currentSection < 0 && currentSection != FQIndexViewSearchSection)
