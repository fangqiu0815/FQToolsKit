...
@implementation FQDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [FQDBModel self]) {
        [self fq_createTable];
    }
}
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        NSDictionary *dic = [self.class fq_getAllProperties];
...
+ (NSDictionary *)fq_getPropertys
{
    ...
    NSArray *theTransients = [[self class] fq_transients];
...
{
    NSDictionary *dict = [self.class fq_getPropertys];
...
{
    __block BOOL res = NO;
    FQDBManage *dwqDB = [FQDBManage shareInstance];
...
+ (BOOL)fq_createTable
{
    ...
    [dwqDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
        NSString *columeAndType = [self.class fq_getColumeAndTypeString];
        ...
        NSDictionary *dict = [self.class fq_getAllProperties];
...
- (BOOL)fq_saveOrUpdate
{
    ...
    return [self fq_update];
}

- (BOOL)fq_saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue
{
    id record = [self.class fq_findFirstByCriteria:[NSString stringWithFormat:@"where %@ = %@",columnName,columnValue]];
    if (record) {
...
{
    //判断是否是FQDBModel的子类
    for (FQDBModel *model in array) {
        if (![model isKindOfClass:[FQDBModel class]]) {
            return NO;
        }
    }
...
+ (BOOL)fq_deleteObjectsWithFormat:(NSString *)format, ...
{
    ...
    return [self fq_deleteObjectsByCriteria:criteria];
}
...
+ (NSArray *)fq_findAll
{
    ...
    [dwqDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            FQDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
+ (instancetype)fq_findFirstWithFormat:(NSString *)format, ...
{
    ...
    return [self fq_findFirstByCriteria:criteria];
}

/** 查找某条数据 */
+ (instancetype)fq_findFirstByCriteria:(NSString *)criteria
{
    NSArray *results = [self.class fq_findByCriteria:criteria];
    if (results.count < 1) {
        return nil;
    }
...
+ (instancetype)fq_findByPK:(int)inPk
{
    ...
    return [self fq_findFirstByCriteria:condition];
}
...
+ (NSArray *)fq_findWithFormat:(NSString *)format, ...
{
    ...
    return [self fq_findByCriteria:criteria];
}
...
+ (NSString *)fq_getColumeAndTypeString
{
