...
@interface FQKeyboardMovingProxy : NSProxy

@property(nonatomic) NSMutableArray *invocations;

-(id)init;

-(void)loadInvocations:(FQKeyboardMoving *)kbMoving;

@end

@implementation FQKeyboardMovingProxy
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector {
    return [FQKeyboardMoving instanceMethodSignatureForSelector:aSelector];
}
...
@implementation FQKeyboardMoving
...
-(id)initWithOwner:(UIView*)txtView
{
    ...
    if (self != nil)
    {
        ...
        [((FQKeyboardMovingProxy *)[FQKeyboardMoving appearance]) loadInvocations:self];
...
{
    static FQKeyboardMovingProxy *proxy = nil;
    
    if (proxy == nil)
    {
        proxy = [[FQKeyboardMovingProxy alloc] init];
    }
    return (FQKeyboardMoving *)proxy;
}
...
@implementation UIView (FQTextInputKeyBoard)
...
- (FQKeyboardMoving *)kbMoving
{
    FQKeyboardMoving *kbMoving = objc_getAssociatedObject(self, ASSOCIATEDOBJECT_KEY_YSTEXTINPUTKEYBOARD_MOVING);
    if (kbMoving == nil)
    {
        kbMoving = [[FQKeyboardMoving alloc] initWithOwner:self];
